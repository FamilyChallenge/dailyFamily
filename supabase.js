...
function pickNextPair(userIds, allChallenges) {
  ...
  // Priorité : d'abord équilibrer qui a le moins souvent été "de" (le rôle le plus visible),
  // puis qui a le moins souvent été "vers", puis la paire exacte en dernier critère.
  const score = ([a, b]) =>
...
async function fetchComments(challengeId) {
  const res = await fetch(
    `${SUPABASE_URL}/rest/v1/challenge_comments?challenge_id=eq.${challengeId}&select=*&order=created_at.asc`,
    { headers: headers() }
  );
  return res.json();
}

async function addComment(challengeId, userId, text) {
  const res = await fetch(`${SUPABASE_URL}/rest/v1/challenge_comments`, {
    method: "POST",
    headers: { ...headers(), Prefer: "return=representation" },
    body: JSON.stringify([{ challenge_id: challengeId, user_id: userId, text }]),
  });
  if (!res.ok) {
    throw new Error(await res.text());
  }
